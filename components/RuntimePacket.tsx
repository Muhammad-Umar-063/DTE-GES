...
export default async function RuntimePacket({
  ...
  if (!packet) {
    return (
      <div className={"card text-center " + (className ?? "")}>
        <h3 className="text-card-title">Runtime Packet</h3>
        <p className="text-body text-text-muted mt-2">
          No packet generated yet for this engagement.
        </p>
      </div>
    );
  }
  ...
  const fields: Array<{ label: string; value: string }> = [
    ...
    { label: "Service Line", value: packet.service_line_id },
    { label: "Workflow State", value: packet.workflow_state },
    {
      label: "Compression",
    ...
    {
      label: "Evidence Refs",
      value: String(fieldCount(packet.evidence_refs?.documents) || fieldCount(packet.evidence_refs)),
    },
    {
      label: "Approvals",
